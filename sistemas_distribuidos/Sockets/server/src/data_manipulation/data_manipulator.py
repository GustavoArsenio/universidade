...
class Gerenciador_clientes():
    ...
    def load_meta_dados(self):
        for cliente_file in os.listdir(self.path_info):
            if '.' == cliente_file[0]: 
                continue

            __FILE    = open(os.path.join(self.path_info,cliente_file),'r+')
    ...
    def add_cliente(self,cliente):
        ...
        if CLIENTE_EXISTE is False:
            logger.debug(self.clientes_dict)
            ...
            logger.debug('Usuario já cadastrado')
    ...
    def remove_cliente(self,nome_cliente):
        ...
        if CLIETEN_EXISTE:
            ...
            logger.debug('usuario Não existe')
        pass
...
if __name__ == '__main__':
    ...
    logger.info(f' Clientes:  {[str(item) for item in gen.clientes_dict.values()]}')
